...
from pyreft import (
    ...
    RedIntervention,
    VIBRedIntervention,
    VIBLobireftIntervention,
    MiniTransformerIntervention
)
...
residual_stream_component_mapping = {
    ...
    "qwen2forcausallm": "model.layers[%s].mlp",
    "gemma2forcausallm": "model.layers[%s].mlp"
}
dtype_mapping = {
...
intervention_mapping = {
    ...
    "VIBRedIntervention": VIBRedIntervention,
    "VIBLobireftIntervention": VIBLobireftIntervention,
}
...
def finetune(
    ...
    print(
        ...
        f"use_compression: {use_compression}, beta: {beta}"
    )
    ...
    if train_dataset is not None:
        ...
        run_name = f"{model_str}.{task}.{intervention_type}.layer_{layers}.rank_{rank}.epoch_{epochs}_lr_{lr}.compression_{use_compression}.beta_{beta}_position_{position}_dropout_{dropout}_residual_{use_residual}_norm_{use_norm}_norm_eps_{norm_eps}.{now}"
    ...
    if "qwen" in model_name.lower():
        # qwen tokenizer does not have unk and pad token
    ...
    reft_model.save(save_directory=f"{output_dir}/{run_name}")
...
def main():
    ...
    parser.add_argument('-use_compression', '--use_compression', action='store_true')
    parser.add_argument('-beta', '--beta', type=float, default=0.1)
    parser.add_argument('-use_residual', '--use_residual', action='store_true')
    parser.add_argument('-use_norm', '--use_norm', action='store_true')
    parser.add_argument('-norm_eps', '--norm_eps', type=float, default=1e-6)
